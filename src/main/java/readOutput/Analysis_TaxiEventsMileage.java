...
public class Analysis_TaxiEventsMileage {
		...
		while ((line = abc.readLine()) != null) {
			...
			if (counter > 1) {
				...
				if (line.contains("occupied_drive")){
					...
					occupied_drive_td = occupied_drive_td + distance - localDistance ;
					
					
					if (fromzone.contains("83") && tozone.contains("83")){
						out_to_out.add(distance - localDistance);
					}
					if (fromzone.contains("83") && !tozone.contains("83")){
						out_to_ams.add(distance - localDistance);
					}
					if (!fromzone.contains("83") && tozone.contains("83")){
						ams_to_out.add(distance - localDistance);
					}
					if(!fromzone.contains("83") && !tozone.contains("83")){
						ams_to_ams.add(distance - localDistance);
					}
